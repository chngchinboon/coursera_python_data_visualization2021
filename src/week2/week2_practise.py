...
def get_county_attributes(svg_file_name):
    ...
    """
    svg_doc = minidom.parse(svg_file_name)  # parseString also exists
    # path_strings = [(path.getAttribute('id'),path.getAttribute('d')) for path in svg_doc.getElementsByTagName('path')] # too convoluted hard to read
...
test_get_attributes(r"assets\week1\USA_Counties_2014.svg")
...
def get_boundary_coordinates(boundary_data):
    ...
    for letter in ignore_list:
        boundary_data = boundary_data.replace(letter, "")

    boundary_data = boundary_data.split()
    # [tuple(map(float, d.split(','))) for d in boundary_data] # hard to read
    # something different in solution: checks length of coordinates == 2, it not will print it out
    boundary_coordinates = [tuple(map(float,d.split(','))) for d in boundary_data]
...
def process_county_attributes(svg_file_name, csv_file_name):
    ...
    county_attribute_list = get_county_attributes(svg_file_name)
    data=[]
    for county in county_attribute_list:
        # compute county centers
        boundary_coords = get_boundary_coordinates(county[1])
        county_center = compute_county_center(boundary_coords)
        data.append([county[0],county_center[0],county_center[1]])

    # overly complicated to read line
    # data = [(county[0],*compute_county_center(get_boundary_coordinates(county[1]))) for county in county_attribute_list]
...
process_county_attributes(r"assets\week1\USA_Counties_2014.svg", r"src\week2\USA_Counties_with_FIPS_and_centers.csv")